...
def cautaredisrec(lista,n,idis):
    if n == 0:
        raise ValueError("Disciplina nu a fost gasita")
    if lista[n-1].getIdd() == idis:
        return lista[n-1]
    else:
        return cautaredisrec(lista,n-1,idis)
...
class DisciplineInMemoryRepository:
    ...
    def cautaredisid(self, iddis):
        ...
        :return:
        """
        return cautaredisrec(self.__discipline,len(self.__discipline),iddis)
        #for dis in self.__discipline:
        #   if dis.getIdd() == iddis:
        #       return dis
    ...
    def cautareiddis(self):
        """
        Cauta id-ul care urmeaza sa fie inserat
        :return:
        """
        ldis = self.__discipline
        if ldis != []:
            idis = int(ldis[-1].getIdd()) + 1
        else:
            idis = 0
        return idis
